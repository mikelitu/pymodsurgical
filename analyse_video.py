...
from depth import load_depth_model_and_transform, calculate_depth_map_from_video
...
def main(
    ...
    depth_model, depth_transform = load_depth_model_and_transform()
    depth_maps = calculate_depth_map_from_video(depth_model, depth_transform, frames)
    ...
    if isinstance(frames, tuple):
        motion_spectrum = (calculate_motion_spectrum(frames[0], K, depth_maps, filtered=filtering, mask=mask, camera_pos="left", save_flow_video=True), calculate_motion_spectrum(frames[1], K, filtered=filtering, mask=mask, camera_pos="right"))
    else:
        motion_spectrum = calculate_motion_spectrum(frames, K, depth_maps, filtered=filtering, mask=mask)
